					...
					if (_.All(i => i != 0)) {
						...
						for (int x = 0; x < sizeX; x++) {
							...
							for (int i = 0; i < sizeY; i++) { 
								if (y - i > 0 && y - i - 1 > 0) { MutateState(x, y - i, State[x, y - i - 1]); } 
							}
						}
					}
				}
		...
		public static void Draw(PlayField field, Tetromino current) {
			...
			while (!Game.Lost) {
				...
				if (!Game.Paused && current.Active) {
					for (int y = 0; y < 4; y++) {
						for (int x = 0; x < 4; x++) {
							if (current.Shape[x, y] == 1) { Console.Write("██"); } else { Console.SetCursorPosition(Console.CursorLeft + 2, Console.CursorTop); }
						}
