...
namespace ConsoleTetris {
    class Program {
        ...
        static void Main(string[] args) {
            ...
            while (!State.Lost) {
                if (!State.Paused) {
                    ...
                    if (current.Ypos + 1 < 15) { current.Ypos++; active = true; } else {
                        active = false;
                        for (int y = 0; y < 4; y++) {
                            for (int x = 0; x < 4; x++) {
        ...
        public static void Draw(int[,] field, Tetromino current) { //TODO: pass the current tetromino into this thread somehow
            while (true) {
                Thread.Sleep(25); //since drawing to console takes roughly 0.25s, delay the thread to reduce flickering
