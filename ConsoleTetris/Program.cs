    ...
    class Program {
        public class Tetromino {
            ...
            private readonly byte[][,] data = {
                new byte[,] { { 0x0 }, { 0xF } }, //I //shape data is stored like this:
        ...
        public class PlayField {
            ...
            public int Cascade() {
                ...
                for (int y = 0; y < sizeY; y++) {
                    ...
                    if (_.All(i => i != 0)) {
                        toShift++;
                        for (int x = 0; x < sizeX; x++) { MutateState(x, y, 0); }
                    }
                    //if (toShift > 0) {
                    //    for (int x = 0; x < sizeX; x++) {
                    //        MutateState(x, y, State[x, y]);
                    //    }
                    //}
                }
                return toShift;
            }
            // if (_.All(i => i == 1)) { rowsToRemove.Add(y); }
    }
        ...
        static void Main(string[] args) {
            int[] scoreMap = { 0, 40, 100, 300, 1200 }; //should be const but vs doesn't wanna play along
                ...
                while (!Game.Lost) {
                    ...
                    if (Console.KeyAvailable) {
                        ...
                        switch (key.Key) {
                            ...
                            case ConsoleKey.Z: { if (current.Active) { current.Rotation++; } break; }
                            case ConsoleKey.X: { if (current.Active) { current.Rotation--; } break; }
                            case ConsoleKey.Escape: { Game.Paused = !Game.Paused; break; }
                            //case ConsoleKey.Backspace: { Game.Lost = true; break; }
                            default: { break; }
                        }
                    }
                }
            ...
            foreach (int i in Game.ShapeBuffer) { Game.ShapeBuffer[i] = random.Next(current.DataLength); }
            while (!Game.Lost) {
                Thread.Sleep(1000 / Game.Level); //use deltaTime instead?
                if (!Game.Paused) {
                    if (!current.Active) {
        ...
        public static void Draw(PlayField field, Tetromino current) {
            ...
            while (!Game.Lost) {
                ...
                if (Game.Paused) { Console.SetCursorPosition(6, 8); Console.Write("{PAUSED}"); } //draw the pause screen dialog
                Console.SetCursorPosition(24, 0); Console.Write($"[ SCORE: {Game.Score,8} ][ LEVEL: {Game.Level,2} ][ LINES: {Game.Lines,4} ]"); //draw score
